...
static int extled_open(struct inode *inode, struct file *file) {
    pr_devel("%s:extled open\n", __func__);
    struct extled_device_info *bdev = container_of(inode->i_cdev, struct extled_device_info, cdev);
    if (bdev==NULL) {
        pr_err("%s:デバイス情報取得失敗\n", __func__);
        return -EFAULT;
    }
...
static int extled_close(struct inode *inode, struct file *file) {
    //実質何もしない
    pr_devel("%s:extled closed\n", __func__);
    return 0;
}

static ssize_t extled_read(struct file *fp, char __user *buf, size_t count, loff_t *f_pos) {
...
static ssize_t extled_write(struct file *fp, const char __user *buf, size_t count, loff_t *f_pos) {
    struct extled_device_info *bdev = fp->private_data;
    char outValue;
    int result;
    int pwm_width;
...
/* ハンドラ　テーブル */
struct file_operations extled_fops = {
    .open     = extled_open,
    .release  = extled_close,
    .read     = extled_read,
    .write    = extled_write,
};

// キャラクタデバイスの登録と、/dev/beep0の生成
static int make_udev(struct extled_device_info *bdev, const char* name) { 
    int ret = 0;
    dev_t dev;
    ...
    cdev_init(&bdev->cdev, &extled_fops);
...
// キャラクタデバイス及び/dev/extled0の登録解除
static void remove_udev(struct extled_device_info *bdev) {
    dev_t dev = MKDEV(bdev->major, MINOR_BASE);
    for (int minor=MINOR_BASE; minor<MINOR_BASE+MINOR_NUM; minor++) {
        /* /sys/class/extled の削除 */
...
static const struct of_device_id of_beep_ids[] = {
    { .compatible = "reterminal_extled" } ,
    { },
};

MODULE_DEVICE_TABLE(of, of_beep_ids);

static int extled_probe(struct platform_device *p_dev) {
    struct device *dev = &p_dev->dev;
    struct extled_device_info *bdev;
    int result;
    ...
    // デバイス情報のメモリ確保と初期化
    bdev = (struct extled_device_info*)devm_kzalloc(dev, sizeof(struct extled_device_info), GFP_KERNEL);
    if (!bdev) {
...
static int extled_remove(struct platform_device *p_dev) {
    struct extled_device_info *bdev = dev_get_drvdata(&p_dev->dev);
    remove_udev(bdev);
    ...
    pr_info("%s:extled driver unloaded\n",__func__);
    return 0;
} 
...
module_platform_driver(extled_driver);
